...
def inverse_CQT(cqt_tensor, sample_rate=22050, n_bins=84, hop_length=256):
    ...
    # Inverse CQT
    waveform = librosa.icqt(
        cqt_complex, sr=sample_rate, hop_length=hop_length)       # (samples,)
...
def plot_stft(spectrogram, sr = 22050, hop_length = 256, log_scale=True):
    ...
    It works with both single and multiple sections of STFT.
    """
    ...
    for idx, sec in enumerate(sections):
        ...
        # magnitude
        stft_magnitude = torch.hypot(real, imag)    # computes sqrt(real**2 + imag**2)
        if log_scale:
            # convert to dB scale
...
def plot_cqt(spectrogram, sr = 22050, hop_length = 256, log_scale=True):
    ...
    Args:
        ...
        fmin: frequenza minima della CQT (per asse frequenze in Hz).
    """
    arr = spectrogram.detach().cpu()
    if arr.ndim == 3:
        sections = [arr]
...
def get_overlap_windows(spectrogram, window_size=WINDOW_SIZE, overlap_frames=OVERLAP_FRAMES):
    ...
    for start_time in range(0, n_time, step_size):
        ...
        section = spectrogram[:, start_time:end_time, :]       # (channels, slice_time, n_freq)

        # padding along time dimension if necessary
    ...
    # stack on a new axis, returning a tensor of shape (n_sections, channels, window_size, n_freq)
...
def sections2spectrogram(sections, original_size, overlap=OVERLAP_FRAMES):
    ...
    # count tensor to average overlapping sections
    ...
    return full[:, :original_size, :]  # return only the original size part
...
def concat_stft_cqt(stft, cqt):
    ...
    Returns:
        torch.Tensor di shape (2, T, F1+F2)
    """
    ...
    # concat on frequency axis (dim=2)
